        ...
        { windowInnerWidth < 887 ? <MobileCVfill hrefSent={"/pages/mytemplate3"} /> : <div className="bg-[#070717] min-h-screen  ">
            {
                token && (
                    <div className=" text-[10px] sm:text-[14px]">
                        <div className="h-[1040px] w-[820px] bg-gray-100 border-gray-500 border-2 border-opacity-40  mx-auto ">
                            ...
                            <div className="flex flex-row w-[100%] h-full border-gray-500 border-2 border-opacity-40   ">
                                ...
                                <div className='w-[30%]  '>
                                    <div className='relative rounded-xl flex justify-center items-center flex-col pt-2'>
                                        {info?.image ? (
                                            <Image  height={120} width={120} src={info?.image} alt="Uploaded Image" className=' object-cover  rounded-xl' />
                                        ) : ""}


                                        <input className="flex items-center justify-center w-[100%]" type="file" accept="image/*" onChange={handleFileUpload} />
                                    </div>
         ...
         items-center  text-gray-700 mb-2 ">
                                    ...
                                    <div className=" h-[35%] border-gray-500 border-2 border-opacity-20 p-2 ">
                                        <input value={info?.skills} onChange={(e) => setInfo({ ...info, skills: e.target.value })} type="text" placeholder='A Title eg. Skills' className=' px-2 font-semibold text-xl mx-auto w-[90%] flex' />
                                        <textarea value={info?.skillsInfo} onChange={(e) => setInfo({ ...info, skillsInfo: e.target.value })} name="" placeholder='You can fill here' id="" className='w-[90%] px-2  h-[80%] font-medium flex mx-auto mt-2' ></textarea>

                                    </div>

                                    <div className=" h-[20%] border-gray-500 border-2 border-opacity-20 p-2 ">
                                        <input value={info?.languages} onChange={(e) => setInfo({ ...info, languages: e.target.value })} type="text" placeholder='A Title eg. Languages' className=' px-2  font-semibold text-xl mx-auto w-[90%] flex' />
                                        <textarea value={info?.languagesInfo} onChange={(e) => setInfo({ ...info, languagesInfo: e.target.value })} name="" id="" placeholder='You can fill here' className='w-[90%] h-[80%] flex mx-auto mt-2 px-2  font-medium' ></textarea>
                                    </div>

                                </div>
                            </div>
                        </div>
                        <Buttons hrefFromParent={"/pages/mytemplate3"} />
                    </div>
                )
            }
            </div> }
